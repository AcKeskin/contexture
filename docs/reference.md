...
> Looking for a deeper architecture view (module map, per-module file graphs, cross-module relations)? Run `/visualise-codemap` (or `node skills/visualise-codemap/visualise-codemap.mjs`) — it generates `.claude/codemap.diagrams.md` locally. That file is gitignored (regenerated per-machine, not shipped), so it won't exist until you run the skill. The tree below is just the folder skeleton.
...
│   ├── <language>/  <domain>/                             # 16 more scopes (see the census below)
│   └── README.md
...
| `clear-context-decision-guard` | SessionStart [clear/compact] | Advisory — scans the prior transcript for unrecapped decision signals after a clear/compact and surfaces a recap nudge via `hookSpecificOutput.additionalContext` at the next session start; never blocks |
...
Hooks ship in default-on bundles — `security`, `gitDiscipline`, `bootstrapDrift`, `agentOutputContracts`, `codemapDirty`, `sessionGuard`, and `rulePrime` — plus two **default-off** automation bundles, `wrapTerminus` and `coordinateAuto` (the opt-in session-terminus + coordinate-autoregister hooks). Bundles are toggled per machine in `~/.claude/hook-config.json` under `enabled.hookBundles` (set a default-off bundle `true` to opt in). All hooks fail-open (exit 0 = allow, exit 2 = block, JSON stdout `hookSpecificOutput.additionalContext` = advise). See [security-hooks](security-hooks.md) and the automation-model note below.

#### The automation model — what fires on its own

The harness is manual-first: most organs are user-invoked (Mode A) and every organ that writes a memory or canonical artefact does so behind an accept/edit/reject gate. Three tiers of automatic behavior sit on top, all advisory (never blocking) and none of which silent-writes:

- **Always-on advisories (default-on).** `rule-prime` primes resolved rules into context at session start + per prompt; `bootstrap-drift-injector` warns when the local install is stale; `clear-context-decision-guard` nudges a recap after a clear/compact left decisions unpersisted. These surface context via `hookSpecificOutput.additionalContext` — they inform, they don't act.
- **Opt-in-per-project.** `codemap-dirty-marker` marks the codemap stale after edits, enabled per project via `.claude/codemap.config.md`.
- **Opt-in automation (default-off).** The `wrapTerminus` and `coordinateAuto` bundles are the newest tier: they *propose* an action (run `/wrap`; register on the coordinate board) at the next session start, and only when their bundle is turned on in `hook-config.json`. Even enabled, they surface a proposal — the write itself still passes the organ's own gate.

The rule throughout: **automatic means auto-propose, never auto-write.** A hook can bring an action to your attention; it cannot take a canonical action on its own. Enabling a default-off bundle changes *what gets proposed to you*, not *what happens without you*.
...
|-------|--------|---------|
| `effort` | `minimal` \| `balanced` \| `thorough` \| `exhaustive` | per-step depth **and** the loop's willingness-to-continue |
| `stopping` | `criteria-met` \| `diminishing-returns` \| `budget` \| `user-anytime` | the stopping posture — `criteria-met` defers to the spec's done-criteria, it does not redefine "done" |
| `ask` | `forks-only` \| `every-step` \| `until-blocked` | a pointer at the shipped ambiguity-depth + act-don't-ask rules, not a new protocol |

It is set four ways, resolved **live > kickoff > overlay-default > implicit-default** (`balanced` / `criteria-met` / `forks-only` — matching current behaviour, so the organ adds nothing until tuned). A live steer is task-scoped and never persists. The **persistent default is a rule file** — [`architectural-rules/universal/autonomy-default.md`](../architectural-rules/universal/autonomy-default.md) — carrying the posture in an `autonomy:` frontmatter block, so it gets the tier cascade/override/disable for free; readers parse that frontmatter directly (`relevance: on-demand`, off the always-on floor).
...
| Reader | Reads | At |
|--------|-------|-----|
| `execute` | `ask` (cadence), `stopping` (§4a done-criteria posture) | the largest consumer seam |
| `checkpoint` | `stopping` | sizes the fit-pass drift question (`user-anytime` → freeze-and-record) |
| `orchestrate` | `effort` + `ask` (Q3 dispatch), `stopping` (Q4 converge) | per-unit, at dispatch only |
| `wrap` | `ask` (interrupt posture for the whole closing ceremony) | once at entry, threaded to every step |
| `coordinate` | `ask` (gates auto mode: `forks-only` → board ops auto, `every-step` → invoked-action) | when auto mode is enabled |
| `spec` / `draft-plan` | set it inline as one optional kickoff question | the implicit surface of the kickoff mode |
| `rule-prime` hook | injects the recall-before-ask line when the contract deviates | every turn, deviation-only |

No hook of its own, no always-on cost, propose-confirm on every write.
...
| `extract-conventions` | Observes a scope's dominant code conventions and writes a project-tier `conventions.md` that overrides universal defaults for that scope. Hybrid detection — mechanical conventions deterministically (`detect.mjs`), semantic conventions model-judged + confidence-flagged. Per-category propose-confirm gate; language-pro-delegated authoring; conflicts with shipped rules surfaced at confirm. The first organ to auto-populate the project tier |
...
| `/close-out` | `skills/close-out` |
| `/wrap` | `skills/wrap` |
| `/work-state` | `skills/work-state` (alias `/status`) |
| `/glossary` | `skills/glossary` |
...
| `/visualise-codemap` | `skills/visualise-codemap` |
...
18 scopes, 105 rule files (`relevance: always` rules in `universal/` form the always-on floor; everything else is phase- or language/domain-gated, so it costs nothing until the work implicates it).
...
| `linux/` (3) | ipc-and-signals, resources-and-fds, syscalls-and-errno |
| `openxr/` (4) | extensions-and-portability, input-actions, lifecycle-and-sessions, spaces-and-tracking |
| `python/` (5) | errors-and-resources, idioms, naming, packaging, typing |
| `rendering/` (5) | color-and-precision, compute-and-gpgpu, frame-graph-and-passes, resource-lifetime, shaders |
...
| `unity/` (8) | component-design, input-actions-on-pointer, meta-files, namespaces-and-imports, serialization-and-inspector, ugui-skill-usage, ui-code, uitoolkit-uss-limits |
| `universal/` (20) | autonomy-default, canonical-commands, change-discipline, code-standards, config-is-truth, deep-modules, docs-and-comments, git, git-workflow, layering, naming, naming-and-comments, no-hardcoded-machine-paths, persist-before-discard, planning-depth, prose-authenticity, session-scope-boundary, skill-auto-fire, solid-and-responsibilities, test-quality |
| `web/` (3) | async, layering, state |
| `webrtc/` (3) | data-channels-and-reliability, media-and-tracks, peer-connection-lifecycle |
...
- `settings.template.json` — full config with the default-on hook bundles (`security`, `gitDiscipline`, `bootstrapDrift`, `agentOutputContracts`, `codemapDirty`, `sessionGuard`, `rulePrime`) plus the two default-off automation bundles (`wrapTerminus`, `coordinateAuto`) and event registrations
