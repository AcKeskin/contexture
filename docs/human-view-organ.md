...
- **draft-plan / blueprint** — the primary callers (the accept/edit/reject review gate + this projection). **deliver** — sibling projector, opposite audience (deliver renders *for the LLM*; human-view renders *for the human*). **AGENTS.md projector / two-doc-tracks** — same humans-vs-agents split, applied to working artefacts. **visualise-codemap** — shares the vault conventions for `--vault`.
