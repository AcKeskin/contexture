...
| Module / skill | How it consumes | | --- | --- | | `resolver` (scope-resolution) **[shipped]** | parses all three files; runs the longest-prefix walk + merge using the precedence + degradation rules above. Algorithm: [`scope-resolution-resolver.md`](scope-resolution-resolver.md). | | `prep` / `discover` integration **[shipped]** | prep runs the resolver before its scope inference (§1); discover consumes the resolved scopes (§6 hard inputs) + `corpus-source` (§4a). See [`scope-resolution-resolver.md`](scope-resolution-resolver.md) § prep/discover integration. | | `boundary-observation` **[shipped]** | prep's drift-watch keys submodule-cross vs subfolder-cross off the filename (`submodule.md` vs `scope.md`) per the asymmetry above; vendored entry warns. | | `review` / `memory-audit` (later) | gain boundary-drift / orphan-manifest finding categories against these formats — deferred. | 