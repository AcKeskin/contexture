...
description: Use when starting feature work that needs isolation from the current workspace, or before executing a non-trivial implementation plan that risks polluting the working tree. Creates an isolated git worktree with directory-selection priority and gitignore safety verification.
---
...
1. Show the exact `.gitignore` line you'd add and note that it needs a commit; ask before proceeding.
2. On confirmation: add the line to `.gitignore` and commit it.
...
**Never:**
- Add to or commit `.gitignore` without surfacing the change and confirming first
...
**Pairs with:**
- `/execute` — when an implementation plan touches enough files to risk polluting the working tree, set up a worktree first.
