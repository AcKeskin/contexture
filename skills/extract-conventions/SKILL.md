...
The convention-extraction organ. **Auto-populates** the project rule tier. Gates on the [rule-prime hook](../../hooks/rule-prime.js) for its output to be load-bearing — without the hook priming the extracted file, it is an inert surface. Feeds [naming-and-comments](../../architectural-rules/universal/naming-and-comments.md), whose audit cites the project-tier conventions this writes.
...
For **semantic** conventions a counter can't capture, judge from the sampled files — but **honestly** (an audit is complete only for the classes it actually checked):
...
**Nothing is written until at least one category is accepted.** Rejecting all categories writes no file and ends the run with "No conventions accepted — nothing written." This is the standard propose-confirm-commit pattern: never auto-write.

### 5. Conflict surfacing vs shipped universal rules

Before writing, resolve the accepted conventions against the **rule overlay** (the shipped universal tier in particular). For each accepted convention, check whether it contradicts a shipped rule — e.g. an `m_` member-prefix convention against `universal/naming.md`'s no-prefix stance, or a next-line brace style against a shipped same-line rule.

When a contradiction is found, surface it **at confirm time** using the `contradicts` / override-with-note pattern:
...
Dispatch protocol — positive scope, hard placement, no recursion:
...
**Never overwrite** an existing project-tier `conventions.md` blind. If one exists for the target scope, show the diff and confirm — or, if the user is refreshing after drift, treat the existing file as a baseline and surface what changed.
...
Do not invoke `/review`, do not commit. The written file is the user's to commit. Do not auto-run `/extract-conventions` again. The conform half is not this skill's job — it is the emergent payoff of the rule-prime hook priming the file + `/review` auditing against it.
...
- **Does not implement the conform mechanism.** Conform = the rule-prime hook (prime) + `/review` (audit) reading this file.
- **Does not detect drift between the file and evolving code.** v1: re-run to refresh.

## Relationship to other organs

- **rule-prime hook** — the consumer that makes this output load-bearing. **Hard dependency** for payoff: the hook primes the extracted file into context. Without it the file is inert (the exact disease the hook fixes).
- **naming-and-comments** — the auditor. The extracted conventions are the project-tier rules its audit cites; conformance precedence is project convention > file-local style > universal default.
- **rule overlay / rules skill** — the tier this writes to and resolves against. `/rules where <key>` shows the written file winning over the shipped default.
...
- **new-agents-md** — downstream beneficiary: real extracted conventions materially improve its `## Conventions` section.
...
- **Written file doesn't prime** — confirm the `rulePrime` hook bundle is enabled (`bootstrap --verify`) and the frontmatter `relevance: when-touching-<lang>` matches the language you're touching. The file is inert without the hook.
