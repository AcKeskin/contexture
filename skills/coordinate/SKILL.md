...
**Liveness — authoritative process check first, transcript mtime as the fallback.** The `Updated` timestamp is *self-reported*: a hung session may have written it just before hanging, and a healthy session mid-long-turn may not have refreshed it. It stays in the table as a human-readable hint but is **not the liveness signal.**

**Primary — the live-session registry (same host only).** `~/.claude/sessions/<pid>.json` files record `{pid, sessionId, cwd, startedAt, procStart, ...}` for running sessions. For each same-host row: scan the registry for an entry whose `sessionId` matches the row's recorded id. Entry found → verify the process: the PID exists **and** its start time is consistent with the registry's `startedAt` (within ~2 minutes — the start-time match is what defeats PID reuse). Both hold → **live, verified** (proof of life, not just activity). PID dead or start-time mismatched (a reused PID) → the session is gone; surface the row as **possibly stale** with "process gone" as the reason. This registry is **undocumented and version-coupled** — treat it defensively: no registry entry for the id, an unparseable file, or an unexpected shape means *fall through to the mtime signal below*, never "dead" and never an error. Absence from an undocumented registry is not evidence of death.

**Fallback — transcript mtime.** Resolve the owning session's transcript from its recorded `session_id` (`~/.claude/projects/<slug>/<session_id>.jsonl`) and `stat` its mtime — a single, fast, cross-platform check that rides this existing read, no background watcher:
...
- **transcript not present on this host** (a synced or SSH-shared home can hold another host's transcripts, whose sessions this machine cannot see) → **unknown**, never stale-and-reclaimable. Absence of the file here is not evidence of death there. Cross-host rows never get the registry check either — PID namespaces don't compare across machines.
...
- **Does not evict on any liveness verdict.** The process check upgrades the *signal* (proof of life on the same host, via the live-session registry with mtime fallback); the *posture* is unchanged — stale warns, the user clears.
