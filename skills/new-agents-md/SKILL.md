...
2. **Architectural rules — resolved form.** Load rules via `skills/discover/SKILL.md`, so the
   tier overlay is already applied: user/company/project overrides + patches in, disables
...
4. **Canonical commands.** If `architectural-rules/universal/canonical-commands.md` exists,
   project its verb→command pins into the Dev-commands section as a *seed* the
   Pass-2 command interview confirms. If absent, the command section starts empty and Pass 2 asks
   cold.
5. **Domain glossary.** If a project-tier glossary (`<repo>/.claude/rules/glossary.md`)
   exists, project its ubiquitous-language terms into the **`## Domain Language`** section —
...
The **Working discipline** section is a fixed block — it projects durable, vendor-neutral
prompting patterns into a form a generic agent can act on. Insert verbatim:
...
- **discover** — Pass 1 reads architectural rules through discover's resolution path so the
  tier overlay is already applied. Do not re-resolve; do not read the raw shipped tree.
...
- **canonical-commands** — producer→consumer seam: canonical-commands pins commands *inside* the
  corpus; this skill projects them *outward*. If the pins exist, the command interview confirms
  from them; if not, it asks cold.
- **org rule tier** — a team-shared org-tier `AGENTS.md` floor layers under the
...
- **architectural-rules tree** — `universal/skill-auto-fire.md` (trigger in description) and
  ...
  The fixed **Working discipline** block re-projects the corpus's durable prompting patterns
  into the emitted AGENTS.md so non-Claude agents inherit them.
