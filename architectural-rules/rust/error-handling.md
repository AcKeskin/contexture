...
- <!-- id: good-error-types --> Error types are meaningful and well-behaved (C-GOOD-ERR):
...
- <!-- id: question-mark-examples --> Examples in rustdoc and tests use `?` for error handling (C-QUESTION-MARK). No `unwrap()`, no `expect()`, no the-old-`try!()`-macro. If `?` requires a return type, wrap the example in `fn main() -> Result<(), Box<dyn Error>>` or use `# Ok::<(), E>(())` hidden setup.
- <!-- id: failure-docs --> Public function docs include `# Errors`, `# Panics`, `# Safety` sections where applicable (C-FAILURE):
