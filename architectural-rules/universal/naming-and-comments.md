...
<!-- id: domain-vocabulary --> Names read in the **domain's vocabulary**, not the machine's. A reader fluent in the problem domain should recognize the term. Prefer `retryBudget` over `intValueForCount2`, `pendingInvoices` over `tmpList`, `settled` over `flagTrue`.
