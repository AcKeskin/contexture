...
<!-- id: no-needless-copy --> Pass sink parameters by value and `std::move` into storage (forwarding pattern). Pass read-only parameters by const-ref. Never copy a resource-owning type where a move or reference suffices. (F.16)
