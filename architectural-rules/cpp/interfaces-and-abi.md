...
<!-- id: no-stl-across-boundary --> Do not pass `std::string`, `std::vector`, or any STL type across an `extern "C"` or DLL boundary. Their layout and allocator are CRT-version-specific; use `const char*`, `T*`+`count`, or the C-style two-call pattern (`capacityInput`, `countOutput`, `elements`). (I.26)
