...
<!-- id: explicit-optional --> Express nullability explicitly with `X | None` (PEP 604). `Optional[X]` is the legacy spelling — soft-deprecated in the typing docs; keep it only for back-compat with pre-3.10 targets. Do not rely on an un-annotated `= None` default to imply it.
