---
description: Scope-dialed fit-and-intent checkpoint — "does this serve the point + cohere + what did I learn?" at diff / module / corpus zoom. Auto-detects scope (+ --scope override); diff scope composes /code-review + a fit-pass; batches findings and routes them.
---
...
- `/checkpoint --scope corpus` — the history + organ-surface audit (decision integrity, intent-vs-shipped, uncaptured lessons, consolidation; organ overlap, dead config, pipeline gaps, vision drift) via `retrospect-core`.
